...
export default function AssessmentPage() {
  ...
  const [loadingStep, setLoadingStep] = useState<string>(
    "Verifying your assessment link…",
  );
  ...
  // Reset to description tab when switching coding questions or tabs
  useEffect(() => {
    setProblemTab("description");
  }, [currentCodingIndex, currentSqlIndex, currentTab]);
  ...
  const [codingResults, setCodingResults] = useState<
    Record<
      string,
      {
        ...
        apex_compiled?: boolean;
        apex_success?: boolean;
        apex_logs?: string;
        performance?: {
  ...
  >({});
  const [problemTab, setProblemTab] = useState<"description" | "submissions">(
    "description",
  );
  ...
  const [apexBlankAnswers, setApexBlankAnswers] = useState<
    Record<string, Record<string, string>>
  >({});
  const [apexBlanksEvaluation, setApexBlanksEvaluation] = useState<{
    results: ApexBlanksEvaluationResult[];
    total_score: number;
    max_score: number;
  } | null>(null);
  ...
  const computeEndTsFromStartedAt = useCallback(
    (startedAtIso: string, totalMinutes: number) => {
  ...
  const bootstrapTimerFromStorage = useCallback(
    (sessionId: string) => {
      try {
    ...
    (sessionId: string) => {
      ...
      try {
        ...
        if (state.version !== 2 && state.codingSolutions) {
          ...
          for (const [challengeId, code] of Object.entries(
            state.codingSolutions,
          )) {
  ...
  const handleNoFace = useCallback(() => {
    ...
    console.warn(
      `[FaceDetection] No face detected (${count}/${MAX_NO_FACE_VIOLATIONS})`,
    );
    ...
    if (count >= MAX_NO_FACE_VIOLATIONS) {
      ...
      if (assessmentData) {
        assessmentsRuntimeApi
          .proctoring(assessmentData.session_id, {
  ...
  const reportProctoringEvent = useCallback(
    async (eventType: string, details?: object) => {
      if (!assessmentData) return;

      try {
        const data = await assessmentsRuntimeApi.proctoring(
        ...
        if (data.terminated) {
          setTerminated(true);
          setWarningMessage(
            "Assessment terminated due to multiple violations.",
          );
          setShowWarning(true);
  ...
  const handleFullscreenChange = useCallback(() => {
    ...
    if (!isFs && !showFullscreenPrompt && !terminated && !completed) {
      ...
      setTerminated(true);
      setWarningMessage(
        "Assessment terminated: You exited fullscreen mode. This is a strict proctoring violation.",
      );
      setShowWarning(true);
  ...
  const handleVisibilityChange = useCallback(() => {
    if (document.hidden && !terminated && !completed && !showFullscreenPrompt) {
      setTerminated(true);
      setWarningMessage(
        "Assessment terminated: You switched tabs or minimized the window. This is a strict proctoring violation.",
      );
      setShowWarning(true);
  ...
  const handleCopyPaste = useCallback(
    (e: ClipboardEvent) => {
      ...
      toast.error(
        "Copy/paste is disabled during the assessment. This violation has been recorded.",
      );
    },
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent) => {
      ...
      if (
        ...
        setTerminated(true);
        setWarningMessage(
          "Assessment terminated: You pressed ESC. This is a strict proctoring violation.",
        );
        setShowWarning(true);
      ...
      if (
        ...
        if (e.key === "F12" || (e.ctrlKey && e.shiftKey)) {
          ...
          toast.error(
            "Developer tools are disabled during the assessment. This violation has been recorded.",
          );
        } else {
          reportProctoringEvent("copy_paste");
          toast.error(
  ...
  useEffect(() => {
    async function loadAssessment() {
      try {
        ...
        const mcqList = Array.isArray(data.mcq_questions)
          ? data.mcq_questions
          : [];
        const codingList = Array.isArray(data.coding_challenges)
          ? data.coding_challenges
          : [];
        const apexBlankList = Array.isArray(data.apex_blanks)
          ? data.apex_blanks
          : [];
        const sqlList = Array.isArray(data.sql_challenges)
          ? data.sql_challenges
          : [];
        ...
        if (savedState) {
          ...
          // Restore MCQ answers
          if (
            savedState.mcqAnswers &&
            Object.keys(savedState.mcqAnswers).length > 0
          ) {
            setMcqAnswers(savedState.mcqAnswers);
          }
          // Restore coding solutions (merge with starter code)
          if (
            savedState.codingSolutions &&
            Object.keys(savedState.codingSolutions).length > 0
          ) {
          ...
          if (
            savedState.codingLanguages &&
            Object.keys(savedState.codingLanguages).length > 0
          ) {
          ...
          if (
            savedState.currentTab &&
            (savedState.currentTab === "mcq" ||
              savedState.currentTab === "coding" ||
              savedState.currentTab === "apex_blanks")
          ) {
            setCurrentTab(savedState.currentTab);
          }
          if (
            savedState.currentMcqIndex !== undefined &&
            savedState.currentMcqIndex >= 0 &&
            savedState.currentMcqIndex < mcqList.length
          ) {
            setCurrentMcqIndex(savedState.currentMcqIndex);
          }
          if (
            savedState.currentCodingIndex !== undefined &&
            savedState.currentCodingIndex >= 0 &&
            savedState.currentCodingIndex < codingList.length
          ) {
            setCurrentCodingIndex(savedState.currentCodingIndex);
          }
          if (
            savedState.currentApexBlankIndex !== undefined &&
            savedState.currentApexBlankIndex >= 0 &&
            savedState.currentApexBlankIndex < apexBlankList.length
          ) {
            setCurrentApexBlankIndex(savedState.currentApexBlankIndex);
          }
          // Restore apex blank answers
          if (
            savedState.apexBlankAnswers &&
            Object.keys(savedState.apexBlankAnswers).length > 0
          ) {
        ...
        if (expiresAt) {
          try {
            ...
            if (Number.isFinite(endTs) && endTs > 0) {
              localStorage.setItem(
                getTimerStorageKey(sessionId),
                String(endTs),
              );
              const remaining = Math.max(
                0,
                Math.floor((endTs - Date.now()) / 1000),
              );
              setTimeRemaining(remaining);
              console.log("[Timer] Bootstrapped from backend expires_at:", {
                expiresAt,
                endTs,
                remaining,
              });
            ...
            console.error(
              "[Timer] Failed to parse expires_at from backend:",
              expiresAt,
              e,
            );
          }
        ...
        // Fallback: compute from started_at if available
          ...
          const endTs = computeEndTsFromStartedAt(
            startedAt,
            Number(data?.total_time_minutes) || 0,
          );
          if (endTs) {
            ...
            setTimeRemaining(remaining);
            console.log("[Timer] Bootstrapped from started_at:", {
              startedAt,
              endTs,
              remaining,
            });
  ...
  const submitApexBlanks = useCallback(async () => {
    ...
    try {
      const data = await assessmentsRuntimeApi.submitApexBlanks(
        assessmentData.session_id,
        apexBlankAnswers,
      );
  ...
  const beginAssessment = useCallback(async () => {
    ...
    try {
      ...
      const minutes =
        ...
        Number(assessmentData?.total_time_minutes) ||
        0;
      ...
      if (endTs) {
        localStorage.setItem(
          getTimerStorageKey(String(assessmentData.session_id)),
          String(endTs),
        );
        const remaining = Math.max(0, Math.floor((endTs - Date.now()) / 1000));
        setTimeRemaining(remaining);
  ...
  const runCode = async (challengeId: string) => {
    ...
    const challenge =
      codingChallenges.find((c) => c.id === challengeId) ||
      sqlChallenges.find((c) => c.id === challengeId);
    if (!challenge) return;

    const lang = challenge.metadata?.is_sql
      ? "sql"
      : codingLanguages[challengeId] || "python3";
    ...
    try {
        ...
        setCodingResults((prev) => ({
          ...
          [challengeId]: {
            ...
            apex_logs: [
              ...
              Array.isArray(data.ai_evaluation.issues) &&
              data.ai_evaluation.issues.length
                ? `\nIssues:\n- ${data.ai_evaluation.issues.join("\n- ")}`
                : "",
              Array.isArray(data.ai_evaluation.improvements) &&
              data.ai_evaluation.improvements.length
      ...
      const errorMessage =
        e?.message || "Failed to run code. Please try again.";
      toast.error(errorMessage);
    } finally {
      setRunningCode(null);
    }
  ...
  const submitCodingSolution = async (challengeId: string) => {
    ...
    try {
      ...
      if (totalPassed === totalTests && totalTests > 0) {
        toast.success(
          `All ${totalTests} test cases passed! Score: ${data.score_percentage?.toFixed(1)}%`,
        );
      } else {
        toast.info(
          `${totalPassed}/${totalTests} passed. Score: ${data.score_percentage?.toFixed(1)}%`,
        );
      }
    } catch (e: any) {
      const errorMessage =
        e?.message || "Failed to submit solution. Please try again.";
      toast.error(errorMessage);
    } finally {
  ...
  const forceSubmitAssessment = async (_reason: "time_expired") => {
    ...
    try {
      if (mcqQuestions.length > 0) {
        const mcqSubmissions = Object.entries(mcqAnswers).map(
        ...
        await assessmentsRuntimeApi.mcqSubmit(
          assessmentData.session_id,
          mcqSubmissions,
        );
      }
      ...
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <Card className="max-w-md w-full">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              Your assessment has been terminated due to multiple proctoring
              violations. Please contact the hiring team for further
              instructions.
            </CardDescription>
          </CardHeader>
        </Card>
          ...
          <Card>
            <CardHeader className="text-center">
              ...
              <CardDescription className="text-base">
                Thank you for completing the technical assessment. Your
                responses have been submitted for evaluation.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <p className="text-sm text-muted-foreground text-center pt-2">
                The hiring team will review your results and contact you
          ...
          <CardContent className="space-y-6">
            <div className="space-y-4">
              <h3 className="font-semibold text-destructive">
                ⚠️ STRICT PROCTORING - READ CAREFULLY:
              </h3>
              <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-4">
                <ul className="space-y-2 text-sm">
                  ...
                  <li className="flex items-start gap-2">
                    ...
                    <span className="font-medium">
                      Face not visible 3 times ={" "}
                      <span className="text-destructive">TERMINATION</span>
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
                    <AlertCircle className="h-4 w-4 mt-0.5 text-warning" />
                    <span className="font-medium">
                      Copy/paste, right-click, DevTools (3 total) ={" "}
                      <span className="text-destructive">TERMINATION</span>
                    </span>
                  </li>
                </ul>
              ...
              <ul className="space-y-2 text-sm text-muted-foreground">
                <li className="flex items-start gap-2">
                  ...
                  <span>
                    Your webcam will be active for AI face detection proctoring
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <Eye className="h-4 w-4 mt-0.5 text-primary" />
                  <span>
                    Keep your face visible at all times — AI monitors your
                    presence
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  ...
                  <span>
                    Fullscreen mode is <strong>mandatory</strong> throughout the
                    entire assessment
                  </span>
                </li>
                <li className="flex items-start gap-2">
            ...
            <div className="bg-muted/50 rounded-lg p-4 space-y-3">
              <p className="text-sm font-semibold">
                Step 1: Grant Camera & Microphone Access
              </p>
              ...
              <div className="flex flex-wrap gap-4 mt-2">
                {hasMcq && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm">
                      {mcqQuestions.length} MCQ Questions
                    </span>
                  </div>
                )}
                {hasApexBlanks && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm">
                      {apexBlanks.length} Apex Questions
                    </span>
                  </div>
                )}
                {hasCoding && !hasApexBlanks && (
                  <div className="flex items-center gap-2">
                    <Code className="h-4 w-4" />
                    <span className="text-sm">
                      {codingChallenges.length} Coding Challenges
                    </span>
                  </div>
                )}
                {hasSql && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm">
                      {sqlChallenges.length} SQL Challenges
                    </span>
                  </div>
                )}
              </div>
  ...
  const mcqProgress =
    mcqQuestions.length === 0
      ? 0
      : (Object.keys(mcqAnswers).length / mcqQuestions.length) * 100;
  const attemptedCodingCount = codingChallenges.filter((challenge) => {
    const lang = challenge.metadata?.is_sql
      ? "sql"
      : codingLanguages[challenge.id] || "python3";
  ...
  const codingProgress =
    codingChallenges.length === 0
      ? 0
      : (codingChallenges.filter((challenge) => {
          const lang = challenge.metadata?.is_sql
            ? "sql"
            : codingLanguages[challenge.id] || "python3";
          const code = (codingSolutions[challenge.id] || {})[lang] || "";
  ...
  const activeTab = validTabs.includes(currentTab)
    ? currentTab
    : validTabs[0] || "mcq";
    ...
    <AssessmentErrorBoundary>
      <div className="min-h-screen bg-background select-none">
                  ...
                  {codingChallenges.length > 0 && (
                    <div className="space-y-1">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium">
                          {attemptedCodingCount} / {codingChallenges.length}{" "}
                          attempted
                        </span>
                      </div>
                      <Progress
                        value={
                          (attemptedCodingCount / codingChallenges.length) * 100
                        }
                        className="h-2"
                      />
                      ...
                      {Object.keys(codingResults).length > 0 && (
                        <div className="flex justify-between text-sm mt-1">
                          ...
                          <span className="font-medium text-success">
                            {Object.values(codingResults).reduce(
                              (sum, r) => sum + r.passed,
                              0,
                            )}{" "}
                            /{" "}
                  ...
                  {apexBlanks.length > 0 && (
                    <div className="space-y-1">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium">
                          {
                            Object.entries(apexBlankAnswers || {}).filter(
                              ([, blanks]) => {
        ...
        <div className="fixed bottom-4 right-4 z-[60]">
          <div
            className="relative rounded-lg overflow-hidden border-2 border-border shadow-lg bg-black"
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-3 space-y-2">
                        <div className="flex items-start gap-2">
                          <XCircle className="h-4 w-4 mt-0.5 text-destructive" />
                          <span>
                            Exiting fullscreen results in immediate termination.
                          </span>
                        </div>
                        <div className="flex items-start gap-2">
                          <XCircle className="h-4 w-4 mt-0.5 text-destructive" />
                          <span>
                            Switching tabs/minimizing the window results in
                            immediate termination.
                          </span>
                        </div>
                        <div className="flex items-start gap-2">
                          <XCircle className="h-4 w-4 mt-0.5 text-destructive" />
                          <span>
                            Clicking outside the assessment window results in
                            immediate termination.
                          </span>
                        </div>
                        <div className="flex items-start gap-2">
                          ...
                          <span>
                            Face not visible 3 times results in termination.
                          </span>
                        </div>
                        <div className="flex items-start gap-2">
                          <AlertTriangle className="h-4 w-4 mt-0.5 text-warning" />
                          <span>
                            Copy/paste, right-click, or DevTools attempts are
                            recorded.
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="bg-muted/50 rounded-lg p-3 space-y-2">
                        <div className="flex items-start gap-2">
                          <Camera className="h-4 w-4 mt-0.5 text-primary" />
                          <span>
                            Keep your camera on and face visible throughout the
                            assessment.
                          </span>
                        </div>
                        <div className="flex items-start gap-2">
                          <Clock className="h-4 w-4 mt-0.5 text-primary" />
                          <span>
                            Timer is shown on the top bar. Submit before time
                            runs out.
                          </span>
                        </div>
                        <div className="flex items-start gap-2">
                          ...
                          <span>
                            Complete MCQ and/or Coding sections as provided for
                            this assessment.
                          </span>
                        </div>
                      </div>
        ...
        <main className="container mx-auto px-4 py-6 relative">
          ...
          <Tabs
            value={activeTab}
            onValueChange={(v) =>
              setCurrentTab(v as "mcq" | "coding" | "apex_blanks" | "sql")
            }
          >
            <TabsList className="flex flex-wrap w-full max-w-3xl mx-auto mb-6">
              {hasMcq && (
                <TabsTrigger
                  value="mcq"
                  className="flex-1 min-w-32 flex justify-center items-center gap-2"
                >
                  <FileQuestion className="h-4 w-4" />
                ...
                <div className="max-w-5xl mx-auto space-y-6">
                  <div className="rounded-lg border bg-muted/30 p-4">
                    <div className="font-medium text-foreground">
                      Apex Syntax Evaluation
                    </div>
                    <div className="text-sm text-muted-foreground mt-1">
                      This section evaluates Apex coding knowledge through
                      syntax-based fill-in-the-blanks questions.
                    </div>
                  </div>

                  <Progress
                    value={
                      ((currentApexBlankIndex + 1) /
                        Math.max(1, apexBlanks.length)) *
                      100
                    }
                    className="h-2"
                  />
                  ...
                  {apexBlanks[currentApexBlankIndex] && (
                    <Card>
                      <CardHeader>
                        <div className="flex items-center justify-between">
                          <CardTitle className="text-lg">
                            Question {currentApexBlankIndex + 1} of{" "}
                            {apexBlanks.length}
                          </CardTitle>
                          <Badge variant="outline">
                            {apexBlanks[currentApexBlankIndex].points} pts
                          </Badge>
                        </div>
                        <CardDescription>
                      ...
                      <CardContent className="space-y-4">
                        <div className="rounded-lg border border-zinc-700 bg-zinc-900 overflow-hidden">
                          <div className="px-3 py-1.5 bg-zinc-800 border-b border-zinc-700 text-xs font-semibold text-zinc-300">
                            Code Snippet
                          </div>
                          <pre className="p-3 text-xs font-mono text-zinc-100 whitespace-pre-wrap overflow-auto max-h-64 leading-relaxed">
                            {apexBlanks[currentApexBlankIndex].code_with_blanks
                              .split("___")
                              .map((part, i, arr) => (
                                <span key={i}>
                                  {part}
                                  {i < arr.length - 1 && (
                                    <span className="inline-flex items-center gap-0.5 mx-0.5 px-1.5 py-0.5 rounded bg-amber-400/20 border border-amber-400/50 text-amber-300 font-bold text-[10px] align-middle">
                                      <span className="inline-flex items-center justify-center h-3.5 w-3.5 rounded-full bg-amber-400/30 text-[9px]">
                                        {i + 1}
                                      </span>
                                      ___
                                    </span>
                        ...
                        <div className="space-y-3">
                          ...
                          {(apexBlanks[currentApexBlankIndex].blanks || []).map(
                            (blank, bi) => (
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            {currentApexBlankIndex >= apexBlanks.length - 1 ? (
                              getNextSectionInfo("apex_blanks") ? (
                                <Button
                                  ...
                                  onClick={() =>
                                    setCurrentTab(
                                      getNextSectionInfo("apex_blanks")!.tab,
                                    )
                                  }
                                >
                                  Next Section →{" "}
                                  {getNextSectionInfo("apex_blanks")!.name}
                                </Button>
                              ) : (
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  setCurrentApexBlankIndex((i) =>
                                    Math.min(apexBlanks.length - 1, i + 1),
                                  )
                                }
                              >
                                Next
                          ...
                          <Button
                            onClick={submitApexBlanks}
                            disabled={submittingApexBlanks}
                          >
                      ...
                      return (
                        ...
                        >
                          <span className="flex items-center gap-1.5">
                            ...
                            {hasResult && !isAccepted && (
                              <AlertCircle className="h-3 w-3" />
                            )}
                            Q{idx + 1}: {renderSafe(c.title)}
                            <span className="text-[10px] opacity-70">
                              ({renderSafe(c.points)}pts)
                            </span>
                          </span>
                        </button>
                      );
                        ...
                        <div className="flex items-center justify-between px-4 py-3 border-b bg-muted/30">
                          <div className="flex items-center gap-2">
                            <h3 className="font-semibold text-sm">
                              {renderSafe(currentCoding.title)}
                            </h3>
                            <Badge variant="outline" className="text-[10px]">
                              {renderSafe(currentCoding.points)} pts
                            </Badge>
                          </div>
                          <div className="flex gap-1">
                            ...
                            >
                              ...
                              {codingResults[currentCoding.id]
                                ? `(${codingResults[currentCoding.id].passed}/${codingResults[currentCoding.id].total})`
                                : ""}
                            </button>
                          </div>
                        </div>
                        ...
                        <div className="flex-1 overflow-auto p-4 space-y-4">
                          {problemTab === "description" ? (
                            <>
                              ...
                              {currentCoding.examples &&
                                currentCoding.examples.length > 0 && (
                                  <div className="space-y-3">
                                    {currentCoding.examples.map((ex, idx) => (
                                      ...
                                      >
                                        <div className="px-3 py-1.5 bg-muted/40 border-b text-xs font-semibold text-muted-foreground">
                                          Example {idx + 1}
                                        </div>
                              ...
                              {currentCoding.constraints && (
                                <div className="rounded-lg border bg-muted/20 overflow-hidden">
                                  ...
                                  <div className="p-3 text-xs font-mono whitespace-pre-wrap text-muted-foreground">
                                    {renderSafe(currentCoding.constraints)}
                                  </div>
                                </div>
                              )}
                              ...
                              {currentCoding.test_cases &&
                                currentCoding.test_cases.length > 0 && (
                                  <div className="space-y-2">
                                    ...
                                    {currentCoding.test_cases.map((tc, idx) => (
                                      <div
                                        key={tc.id || idx}
                                        className="bg-muted/30 rounded-lg p-2.5 text-xs font-mono border"
                                      >
                                        <div>
                                          <span className="text-muted-foreground">
                                            Input:
                                        ...
                                        <div>
                                          ...
                                          {renderSafe(tc.expected_output)}
                                        </div>
                                      </div>
                                    ))}
                                    <p className="text-[10px] text-muted-foreground italic">
                                      Hidden test cases will be evaluated on
                                      submission.
                                    </p>
                                  </div>
                                )}
                          ...
                          codingResults[currentCoding.id] ? (
                            <div className="space-y-3">
                              ...
                              {!(
                                codingResults[currentCoding.id]
                                  .compilation_error ||
                                codingResults[currentCoding.id].runtime_error
                              ) && (
                                <motion.div
                                  initial={{ opacity: 0, y: -10 }}
                                  animate={{ opacity: 1, y: 0 }}
                                  className={`rounded-lg p-4 border ${
                                    codingResults[currentCoding.id].passed ===
                                      codingResults[currentCoding.id].total &&
                                    codingResults[currentCoding.id].total > 0
                                ...
                                >
                                  <div className="flex items-center justify-between">
                                    ...
                                    <span className="text-2xl font-bold">
                                      {safeNumber(
                                        codingResults[currentCoding.id].score,
                                      ).toFixed(0)}
                                      %
                                    </span>
                                  </div>
                                  <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
                                    ...
                                    {codingResults[currentCoding.id].performance
                                      ?.avg_time_ms && (
                              ...
                              <div className="space-y-1.5">
                                {codingResults[currentCoding.id].results.map(
                                  (result, idx) => (
                                    <motion.div
                                      ...
                                      <div className="mt-1.5 pt-1.5 border-t border-dashed text-muted-foreground space-y-0.5">
                                        ...
                                        {result.stdout && (
                                          <div className="mt-1 pt-1 border-t border-dotted border-zinc-700">
                                            ...
                                            <pre className="text-[11px] font-mono text-zinc-100 whitespace-pre-wrap bg-black/30 rounded px-2 py-1 mt-0.5 max-h-20 overflow-auto">
                                              {renderSafe(result.stdout)}
                                            </pre>
                                          </div>
                                        )}
                                        {result.stderr && (
                                          <div className="mt-1 pt-1 border-t border-dotted border-zinc-700">
                                            ...
                                            <pre className="text-[11px] font-mono text-red-400/90 whitespace-pre-wrap bg-red-950/20 rounded px-2 py-1 mt-0.5 max-h-20 overflow-auto">
                                              {renderSafe(result.stderr)}
                                            </pre>
                                          </div>
                                        )}
                                      </div>
                      ...
                      <div className="flex flex-col h-full min-h-0 border rounded-lg overflow-hidden bg-card">
                        ...
                        <div className="flex items-center justify-between px-3 py-2 border-b bg-muted/30">
                          <div className="flex items-center gap-2">
                            <Select
                              value={
                                codingLanguages[currentCoding.id] || "python3"
                              }
                              onValueChange={(lang) => {
                                setCodingLanguages((prev) => ({
                                  ...prev,
                                  [currentCoding.id]: lang,
                                }));
                                setCodingSolutions((prev) => {
                                  const existing =
                                    prev[currentCoding.id]?.[lang];
                                  if (existing !== undefined) return prev;
                                  const starter =
                                    currentCoding.starter_code?.[lang] || "";
                                  return {
                                    ...prev,
                                    [currentCoding.id]: {
                            ...
                            <Button
                              ...
                              onClick={() => resetCode(currentCoding.id)}
                            >
                              Reset
                            </Button>
                          </div>
                          <div className="flex gap-1.5">
                            ...
                            <Button
                              ...
                              onClick={() =>
                                submitCodingSolution(currentCoding.id)
                              }
                              disabled={
                                runningCode === currentCoding.id ||
                                submittingCode === currentCoding.id
                        ...
                        >
                          <Editor
                            ...
                            theme="vs-dark"
                            value={
                              (codingSolutions[currentCoding.id] || {})[
                                codingLanguages[currentCoding.id] || "python3"
                              ] || ""
                            }
                            onChange={(value) => {
                        ...
                        <AnimatePresence>
                          {(runningCode === currentCoding.id ||
                            ...
                            >
                              ...
                              {submittingCode
                                ? "Evaluating against all test cases..."
                        ...
                        {codingResults[currentCoding.id] &&
                          ...
                          (() => {
                            ...
                            const activeIndex =
                              activeTestCaseTab < results.length
                                ? activeTestCaseTab
                                : 0;
                            ...
                            return (
                              <div
                                className="flex flex-col flex-1 bg-zinc-950 overflow-hidden"
                                style={{ minHeight: 250 }}
                              >
                                {/* Header */}
                                  ...
                                  {!compilationError && results.length > 0 && (
                                    <>
                                      ...
                                      <div className="flex items-center gap-3">
                                        <span
                                          className={`text-lg font-bold ${result.passed === result.total ? "text-green-500" : "text-red-500"}`}
                                        >
                                          {result.passed === result.total
                                            ? "Accepted"
                                            : "Wrong Answer"}
                                      ...
                                      <div className="flex flex-wrap gap-2 pt-1 pb-2">
                                        {results.map((r, idx) => (
                                          ...
                                          >
                                            <div
                                              className={`h-1.5 w-1.5 rounded-full ${r.status !== "CE" && r.passed ? "bg-green-500" : "bg-red-500"}`}
                                            />
                                            Case {idx + 1}
                                          </button>
                                        ))}
                                      ...
                                      {activeResult && (
                                        <div className="space-y-3">
                                          ...
                                          <div>
                                            ...
                                            <div className="bg-zinc-900 border border-zinc-800 rounded px-3 py-2">
                                              <pre
                                                className={`text-xs font-mono whitespace-pre-wrap ${!activeResult.passed && !activeResult.stderr ? "text-red-400" : "text-zinc-300"}`}
                                              >
                                                {renderSafe(
                  ...
                  <div className="flex justify-start gap-4">
                    ...
                    {currentCodingIndex >= codingChallenges.length - 1 ? (
                      getNextSectionInfo("coding") ? (
                        <Button
                          onClick={() =>
                            setCurrentTab(getNextSectionInfo("coding")!.tab)
                          }
                        >
                          Next Section → {getNextSectionInfo("coding")!.name}
                        </Button>
                      ) : (
                        <Button disabled>Next Challenge</Button>
                      )
                    ) : (
                      <Button
                        onClick={() =>
                          setCurrentCodingIndex((prev) =>
                            Math.min(codingChallenges.length - 1, prev + 1),
                          )
                        }
                      >
                        Next Challenge
                ...
                <div className="max-w-7xl mx-auto space-y-4">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <div className="ml-auto">
                      <Progress
                        value={
                          ((currentSqlIndex + 1) / sqlChallenges.length) * 100
                        }
                    ...
                    <div className="grid lg:grid-cols-2 gap-4 h-[74vh] min-h-[620px]">
                      ...
                      <div className="flex flex-col h-full min-h-0 border rounded-lg overflow-hidden bg-card">
                        ...
                        <div className="flex items-center justify-between px-4 py-3 border-b bg-muted/30">
                          <div className="flex items-center gap-2">
                            <h3 className="font-semibold text-sm">
                              {renderSafe(sqlChallenges[currentSqlIndex].title)}
                            </h3>
                            <Badge variant="outline" className="text-[10px]">
                              {renderSafe(
                                sqlChallenges[currentSqlIndex].points,
                              )}{" "}
                              pts
                            </Badge>
                          </div>
                          <div className="flex gap-1">
                            ...
                            >
                              ...
                              {codingResults[sqlChallenges[currentSqlIndex].id]
                                ? `(${codingResults[sqlChallenges[currentSqlIndex].id].passed}/${codingResults[sqlChallenges[currentSqlIndex].id].total})`
                                : ""}
                            </button>
                          </div>
                        </div>
                        ...
                        <div className="flex-1 overflow-auto p-4 space-y-4">
                          {problemTab === "description" ? (
                            <>
                              ...
                              {sqlChallenges[currentSqlIndex].examples &&
                                sqlChallenges[currentSqlIndex].examples.length >
                                  0 && (
                                  <div className="space-y-3">
                                    {sqlChallenges[
                                      currentSqlIndex
                              ...
                              {/* SQL Specific Fields */}
                              {sqlChallenges[currentSqlIndex].metadata
                                ?.is_sql && (
                                <>
                                  {sqlChallenges[currentSqlIndex].metadata
                                    .db_schema && (
                              ...
                              {sqlChallenges[currentSqlIndex].test_cases &&
                                sqlChallenges[currentSqlIndex].test_cases
                                  .length > 0 && (
                                  <div className="space-y-2">
                                    <h4 className="font-semibold text-xs text-muted-foreground uppercase tracking-wide">
                                  ...
                                  >
                                    <div className="flex items-center justify-between">
                                      ...
                                      <div className="flex items-center gap-3 text-[10px] text-muted-foreground font-sans">
                                        ...
                                        {result.memory_used && (
                                          <span>
                                            {renderSafe(result.memory_used)} KB
                                          </span>
                                        )}
                                      </div>
                                    </div>
                                      ...
                                      {result.is_hidden ? null : (
                                        <>
                                          ...
                                          <div>
                                            ...
                                            {renderSafe(result.expected_output)}
                                          </div>
                                        </>
                                      )}
                                      <div
                                      ...
                                      >
                                        {result.error ? (
                                          ...
                                          <>
                                            ...
                                            {renderSafe(result.actual_output) ||
                                              "N/A"}
                                          </>
                                        )}
                                      </div>
                        ...
                        <div className="flex items-center justify-between px-3 py-2 border-b bg-muted/30">
                          <div className="flex items-center gap-2">
                            <div className="flex flex-col leading-tight">
                              <div className="text-xs font-semibold flex items-center gap-1">
                                <Code className="h-3 w-3" /> MySQL Database
                                Engine
                              </div>
                            ...
                            <Button
                              ...
                              className="h-7 text-[10px] text-muted-foreground"
                              onClick={() =>
                                resetCode(sqlChallenges[currentSqlIndex].id)
                              }
                            >
                              Reset
                            </Button>
                          ...
                          <div className="flex gap-1.5">
                            <Button
                              ...
                              className="h-7 text-xs"
                              onClick={() =>
                                runCode(sqlChallenges[currentSqlIndex].id)
                              }
                              disabled={
                                runningCode ===
                                  sqlChallenges[currentSqlIndex].id ||
                  ...
                  <div className="flex justify-start gap-4">
                    <Button
                      variant="outline"
                      onClick={() =>
                        setCurrentSqlIndex((prev) => Math.max(0, prev - 1))
                      }
                    ...
                    {currentSqlIndex >= sqlChallenges.length - 1 ? (
                      getNextSectionInfo("sql") ? (
                        <Button
                          onClick={() =>
                            setCurrentTab(getNextSectionInfo("sql")!.tab)
                          }
                        >
                      ...
                      <Button
                        onClick={() =>
                          setCurrentSqlIndex((prev) =>
                            Math.min(sqlChallenges.length - 1, prev + 1),
                          )
                        }
                      >
                        Next Challenge
