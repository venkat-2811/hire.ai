 ...
 */
function shuffleMcqOptions(q: any): any {
...
// ── PDF Generation for Offer Letter ─────────────────────────────────────────
function generateOfferLetterPDF(data: {
  ...
  doc.text('OFFER LETTER', pageWidth / 2, 30, { align: 'center' });

  // Company name in header
  doc.setFontSize(14);
  doc.setFont('helvetica', 'normal');
  doc.text(data.companyName, pageWidth / 2, 42, { align: 'center' });
  ...
  const content = [
    ...
    '• Onboarding process',
    '',
    'Please review this offer letter carefully. To accept this offer, please click the "Accept Offer Letter" button in the email you received and follow the instructions to provide your digital signature.',
    '',
    'We look forward to welcoming you to the team and are excited about the contributions you will bring to our organization.',
    '',
    'If you have any questions, please do not hesitate to reach out to our HR team.',
  ];

  // Add CTC if provided
  if (data.ctc) {
    content.splice(8, 0, `• Annual CTC: ${data.ctc}`);
  }

  // Add time period if provided
  if (data.timePeriodYears || data.timePeriodMonths) {
    const period = [];
    if (data.timePeriodYears) period.push(`${data.timePeriodYears} year${data.timePeriodYears > 1 ? 's' : ''}`);
    if (data.timePeriodMonths) period.push(`${data.timePeriodMonths} month${data.timePeriodMonths > 1 ? 's' : ''}`);
    content.splice(8, 0, `• Notice Period: ${period.join(' and ')}`);
  }
  ...
  doc.setFont('helvetica', 'normal');
  doc.text('Talent Acquisition Team', margin, y);
  y += 6;
  ...
  doc.text(`© ${new Date().getFullYear()} ${data.companyName}. All rights reserved.`, pageWidth / 2, pageHeight - 15, { align: 'center' });
  doc.text('Generated by Hire.AI - Intelligent Hiring Platform', pageWidth / 2, pageHeight - 8, { align: 'center' });
...
${resumeText.slice(0, 8000)}`;
    ...
    if (req.method === 'POST' && segments.length === 2 && segments[1] === 'send-offer-letter') {
      ...
      const { candidate_ids, job_id, company_name, ctc, time_period_years, time_period_months } = req.body as {
      ...
      for (const c of candidates) {
        try {
          // Generate unique token for offer acceptance
          const offerToken = crypto.randomBytes(32).toString('base64url');
          const offerId = uuidv4();

          // Store offer letter details in database
          const { error: offerError } = await supabase.from('offer_letters').insert({
            id: offerId,
            candidate_id: c.id,
            job_id,
            token: offerToken,
            ...
            ctc: ctc || null,
            time_period_years: time_period_years || null,
            time_period_months: time_period_months || null,
            status: 'pending',
            sent_at: new Date().toISOString(),
          });

          if (offerError) {
            console.error('[send-offer-letter] Failed to store offer letter:', offerError.message);
            throw new Error('Failed to create offer letter record');
          }

          // Generate PDF
          const pdfBase64 = generateOfferLetterPDF({
            candidateName: c.full_name,
            jobTitle: job.title,
            companyName: resolvedCompany,
            ctc,
            timePeriodYears: time_period_years,
            timePeriodMonths: time_period_months,
          });

          // Send email with PDF attachment and Accept button
          const acceptUrl = `${frontendUrl}/offer-accept/${encodeURIComponent(offerToken)}`;
...
<body style="margin:0;padding:0;background:#f4f6f9;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f6f9;padding:40px 0;">
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <tr><td style="padding:48px;">
          ...
          <div style="background:#F0FDF4;border:1px solid #BBF7D0;border-radius:8px;padding:20px 24px;margin:0 0 28px;text-align:center;">
            ...
            ${ctc ? `<p style="margin:8px 0 0;font-size:14px;color:#047857;">Annual CTC: ${ctc}</p>` : ''}
          </div>
          <p style="margin:0 0 20px;font-size:15px;color:#4B5563;line-height:1.7;">
          ...
          <p style="margin:0 0 20px;font-size:15px;color:#4B5563;line-height:1.7;">
            Please find the complete offer letter document attached to this email.
          </p>
          <div style="text-align:center;margin:32px 0;">
            <a href="${acceptUrl}" style="background:linear-gradient(135deg,#4F46E5 0%,#7C3AED 100%);color:white;padding:16px 32px;text-decoration:none;border-radius:8px;display:inline-block;font-size:16px;font-weight:600;box-shadow:0 4px 12px rgba(79,70,229,0.3);">Accept Offer Letter</a>
          </div>
          <p style="margin:0 0 32px;font-size:15px;color:#4B5563;line-height:1.7;">
        ...
        <tr><td style="background:#F9FAFB;padding:24px 48px;text-align:center;border-top:1px solid #E5E7EB;">
          ...
          await sendEmail(
            ...
            offerHtml,
            [{
              filename: `Offer_Letter_${c.full_name.replace(/\s+/g, '_')}_${job.title.replace(/\s+/g, '_')}.pdf`,
              content: pdfBase64,
    ...
    // GET /api/offer-accept/:token - Get offer letter details for acceptance page
    if (req.method === 'GET' && segments[0] === 'offer-accept' && segments.length === 2) {
      const token = decodeURIComponent(segments[1] || '').trim();
      
      const { data: offer, error } = await supabase
        .from('offer_letters')
        .select('*, candidates(full_name, email), job_descriptions(title)')
        .eq('token', token)
        .single();

      if (error || !offer) {
        return notFound(res, 'Offer letter not found or link expired');
      }

      if (offer.status !== 'pending') {
        return badRequest(res, `This offer has already been ${offer.status}`);
      }

      return ok(res, {
        candidate_name: offer.candidates.full_name,
        candidate_email: offer.candidates.email,
        job_title: offer.job_descriptions.title,
        company_name: offer.company_name,
        ctc: offer.ctc,
        time_period_years: offer.time_period_years,
        time_period_months: offer.time_period_months,
        sent_at: offer.sent_at,
      });
    }

    // POST /api/offer-accept/:token - Accept offer letter with digital signature
    if (req.method === 'POST' && segments[0] === 'offer-accept' && segments.length === 2) {
      const token = decodeURIComponent(segments[1] || '').trim();
      const { signature_full_name } = req.body as { signature_full_name?: string };

      if (!signature_full_name || signature_full_name.trim().length < 2) {
        return badRequest(res, 'Please provide your full name as your digital signature');
      }
      ...
      // Update offer letter with acceptance
      const { error: updateError } = await supabase
        .from('offer_letters')
        .update({
          status: 'accepted',
          accepted_at: new Date().toISOString(),
          signature_full_name: signature_full_name.trim(),
          updated_at: new Date().toISOString(),
        })
        .eq('id', offer.id);

      if (updateError) {
        console.error('[offer-accept] Failed to update offer:', updateError.message);
      ...
      await supabase
        .from('job_applications')
        .update({ final_status: 'offer_accepted', updated_at: new Date().toISOString() })
        .eq('candidate_id', offer.candidate_id)
        .eq('job_id', offer.job_id);

      return ok(res, { success: true, message: 'Offer accepted successfully' });
    }
